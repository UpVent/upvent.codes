                                ...
                                @foreach ($data as $item)
                                    <tr>
                                        <td class="px-6 py-4 text-sm whitespace-no-wrap">
                                            {{$item->title}}
                                            {!! $item->is_default_home ? '<span class="text-green-400 text-xs font-bold"> Default Homepage <i class="items-center pl-2 pb-1 bi bi-house-fill"></i></span>':'' !!}
                                            {!! $item->is_default_not_found ? '<span class="text-red-400 text-xs font-bold"> Default Error 404 Page <i class="items-center pl-2 pb-1 bi bi-x-circle-fill"></i></span>':'' !!}
                                        </td>
                                        <td class="px-6 py-4 text-sm whitespace-no-wrap">
                                            <a
                                                target="_blank"
                                        ...
                                        <td class=:w"px-6 py-4 text-sm">
                                            <x-jet-button wire:click="updateShowModal({{ $item->id }})">
                                                {{ __('Update') }}
                                                <i class="items-center pl-2 pb-1 bi bi-pencil-square"></i>
                                            </x-jet-button>
                                            <x-jet-danger-button wire:click="deleteShowModal({{ $item->id }})">
                                                {{ __('Delete') }}
                                                <i class="items-center pl-2 pb-1 bi bi-x-circle-fill"></i>
                                            </x-jet-button>
                                        </td>
                                    </tr>
                                @endforeach
        ...
        <x-slot name="content">
            ...
            <div class="mt-4">
                <label class="inline-flex items-center mt-3">
                    <input class="form-checkbox rounded-sm shadow-lg h-5 w-5 text-blue-600" type="checkbox" value="{{ $isSetToDefaultHomePage }}" wire:model="isSetToDefaultHomePage" />
            ...
            <div class="mt-4">
                <label class="inline-flex items-center mt-3">
                    <input class="form-checkbox rounded-sm shadow-lg h-5 w-5 text-blue-600" type="checkbox" value="{{ $isSetToDefaultNotFoundPage }}" wire:model="isSetToDefaultNotFoundPage" />
